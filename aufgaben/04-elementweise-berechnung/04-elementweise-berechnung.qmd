...
Laden Sie das Grundgerüst für die Programmierung zu diesem Paket als [Jupyter-Notebook](04-elementweise-berechnung-t1.ipynb)  oder [Quarto-Datei](04-elementweise-berechnung-t1.qmd) herunter.
...
Erstellen und testen Sie die Funktion `assemble_kr(m)`, die für ein Netz `m` die globale Steifigkeitsmatrix $\mathbf{K}$ und den globalen Lastvektor $\mathbf{r}$ zurück gibt. Feder und Einzellast sollen an dieser Stelle noch nicht berücksichtigt werden.
...
Kopieren Sie ihr Notebook für das vorherige Aufgabenblatt und ändern Sie die Funktion `pile_fem` dahingehend, dass die Berechnung von $\hat{\mathbf{u}}$ mithilfe der Funktionen aus diesem Aufgabenblatt erfolgt.