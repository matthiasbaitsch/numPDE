...
1. Berechnen Sie in der Datei `heat_plate.ipynb` die Näherungslösung der Temperaturverteilung für die Kreisplatte mit $r=2.2$, frei gewählten Parametern $\lambda$ und $w$ sowie einer vorgebenen Temperatur $\theta^* = 0$ auf dem Rand. Vergleichen Sie die maximale Temperatur mit der exakten Lösung
    $$
